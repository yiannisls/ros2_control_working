  ...
  controller_manager_msgs__msg__HardwareInterface__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    controller_manager_msgs__msg__HardwareInterface * data =
      (controller_manager_msgs__msg__HardwareInterface *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!controller_manager_msgs__msg__HardwareInterface__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          controller_manager_msgs__msg__HardwareInterface__fini(&data[i]);
        }
        free(data);
        return false;
      }
