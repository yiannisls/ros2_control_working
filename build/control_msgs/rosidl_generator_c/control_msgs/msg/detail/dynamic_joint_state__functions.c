  ...
  control_msgs__msg__DynamicJointState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    control_msgs__msg__DynamicJointState * data =
      (control_msgs__msg__DynamicJointState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!control_msgs__msg__DynamicJointState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          control_msgs__msg__DynamicJointState__fini(&data[i]);
        }
        free(data);
        return false;
      }
