  ...
  control_msgs__msg__JointTolerance__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    control_msgs__msg__JointTolerance * data =
      (control_msgs__msg__JointTolerance *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!control_msgs__msg__JointTolerance__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          control_msgs__msg__JointTolerance__fini(&data[i]);
        }
        free(data);
        return false;
      }
