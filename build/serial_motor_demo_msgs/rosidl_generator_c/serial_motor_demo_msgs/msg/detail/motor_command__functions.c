  ...
  serial_motor_demo_msgs__msg__MotorCommand__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    serial_motor_demo_msgs__msg__MotorCommand * data =
      (serial_motor_demo_msgs__msg__MotorCommand *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!serial_motor_demo_msgs__msg__MotorCommand__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          serial_motor_demo_msgs__msg__MotorCommand__fini(&data[i]);
        }
        free(data);
        return false;
      }
